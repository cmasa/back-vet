...
exports.update = async (id, { nombre, especie, raza, edad, peso, inactivo }) => {
  await pool.query(
    'UPDATE mascotas SET nombre = ?, especie = ?, raza = ?, edad = ?, peso = ?, inactivo = ? WHERE id = ?',
    [nombre, especie, raza, edad, peso, inactivo, id]
  );
  return { id, nombre, especie, raza, edad, peso, inactivo };
};